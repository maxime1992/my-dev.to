...
import {
  ReflectorMapIndexToSameIndex,
  ReflectorService
} from './reflector.service';

describe('ReflectorService', () => {
  it(`should thow an error if the alphabet is not complete or valid`, () => {
    expect(() => new ReflectorService('incorrect alphabet')).toThrowError(
      new InvalidAlphabet()
    );
  });
  ...
  it(`should thow an error a letter is mapping to itself`, () => {
    expect(
      () => new ReflectorService('zabcdefghijklmnopqrstuvwxy')
    ).not.toThrow();
    expect(
      () => new ReflectorService('zbacdefghijklmnopqrstuvwxy')
    ).toThrowError(new ReflectorMapIndexToSameIndex());
  });
