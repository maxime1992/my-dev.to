...
import { EnigmaMachineRequires3Rotors, EnigmaMachineService } from './enigma-machine.service';
...
describe(`EnigmaMachineService`, () => {
  it(`should throw an error if there's not 3 rotors`, () => {
    expect(() => new EnigmaMachineService([], null as any)).toThrowError(new EnigmaMachineRequires3Rotors());
  });
  ...
  it(`should throw an error if at least one of the 3 rotors are not instances of the EnigmaRotorService`, () => {
    ...
    expect(
      () => new EnigmaMachineService([new Random() as any, new Random() as any, new Random() as any], null as any),
